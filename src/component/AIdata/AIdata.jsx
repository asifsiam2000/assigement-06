      ...
      <div className="max-w-[1200px] mx-auto container p-10 flex justify-center items-center flex-col space-y-4 mb-5">
        ...
        <div className="inline-flex items-center p-1 border border-gray-100 rounded-full bg-white shadow-sm gap-1">
          {/* <button className="btn bg-linear-to-r from-[#4F39F6] to-[#9514FA] text-white border-none rounded-full"> Get Started</button> */}
                  ...
                  <button onClick={() => setSelectedType("cart")} className={`btn border-none rounded-full ${selectedType === "cart" ? "bg-linear-to-r from-[#4F39F6] to-[#9514FA] text-white" : ""} `}> Cart({purchesType.length == 0 ? "" : purchesType.length})</button>
          ...
          <div className="max-w-[1200px] mx-auto container">
              {selectedType === "products" ? (<AIdataCard AIdatas={AIdatas} purchesType={purchesType} setPurchesType={setPurchesType}></AIdataCard>) : (<SelectedData purchesType={purchesType} setPurchesType = {setPurchesType}></SelectedData>)
              }
          </div>
